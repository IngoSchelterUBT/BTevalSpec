...
#        exdict["fix"            ] = self.fix
#        exdict["energy"         ] =    np.float64(self.energy) #Explicit casting to float is necesary for ruaml.dump
#        exdict["energyErr"      ] =    np.float64(self.energyErr) #Explicit casting to float is necesary for ruaml.dump
#        exdict["erange"         ] = [  np.float64(self.erange[i]) for i in range(2)]
#        exdict["phase"          ] =    np.float64(self.phase)
#        exdict["phaseErr"       ] =    np.float64(self.phaseErr)
#        exdict["tmod"           ] =    np.float64(self.tmod)
#        exdict["dipoles"        ] = [[ np.float64(self.dipoles        [i][j])    for j in range(len(self.dipoles     [i]))] for i in range(len(self.dipoles   ))]
#        exdict["dipolesErr"     ] = [[ np.float64(self.dipolesErr     [i][j])    for j in range(len(self.dipolesErr  [i]))] for i in range(len(self.dipolesErr))]
#        exdict["ampl"           ] = [[[np.float64(self.ampl           [i][j][k]) for k in range(len(self.ampl     [i][j]))] for j in range(len(self.ampl   [i]))] for i in range(len(self.ampl   ))]
#        exdict["amplErr"        ] = [[[np.float64(self.amplErr        [i][j][k]) for k in range(len(self.amplErr  [i][j]))] for j in range(len(self.amplErr[i]))] for i in range(len(self.amplErr))]
#        exdict["dipole"         ] = [  np.float64(self.dipole         [i])       for i in range(len(self.dipole         ))]
#        exdict["dipoleErr"      ] = [  np.float64(self.dipoleErr      [i])       for i in range(len(self.dipoleErr      ))]
#        exdict["transdensDipole"] = [  np.float64(self.transdensDipole[i])       for i in range(len(self.transdensDipole))]
#        exdict["strength"       ] =    np.float64(self.strength)
#        exdict["strengthErr"    ] =    np.float64(self.strengthErr)
#        exdict["strengthEped"   ] = [  np.float64(self.strengthEped)         for i in range(len(self.strengthEped   ))]
#        exdict["strengthErrEped"] = [  np.float64(self.strengthEpedErr)      for i in range(len(self.strengthEpedErr))]
#        exdict["strengths"      ] = [  np.float64(self.strengths   [i])      for i in range(len(self.strengths      ))]
#        exdict["strengthsErr"   ] = [  np.float64(self.strengthsErr[i])      for i in range(len(self.strengthsErr   ))]
#        exdict["eped"           ] = [  np.float64(self.eped[i])              for i in range(len(self.eped           ))]
#        exdict["epedErr"        ] = [  np.float64(self.epedErr[i])           for i in range(len(self.epedErr        ))]
#        exdict["signifFit"      ] =    np.float64(self.signifFit)
#        exdict["signifErr"      ] =    np.float64(self.signifErr)
#        exdict["signifAng"      ] =    np.float64(self.signifAng)
#        exdict["signifExc"      ] =    np.float64(self.signifExc)
#        exdict["signifRng"      ] =    np.float64(self.signifRng)
#        exdict["signifPha"      ] =    np.float64(self.signifPha)
#        return exdict
...
#        exdict["name"           ] = self.name
#        exdict["fix"            ] = self.fix
#        exdict["energy"         ] =    self.energy.astype(float) #Explicit casting to float is necesary for ruaml.dump
#        exdict["energyErr"      ] =    self.energyErr.astype(float) #Explicit casting to float is necesary for ruaml.dump
#        exdict["erange"         ] = [  self.erange[i].astype(float) for i in range(2)]
#        exdict["phase"          ] =    self.phase.astype(float)
#        exdict["phaseErr"       ] =    self.phaseErr.astype(float)
#        exdict["tmod"           ] =    self.tmod.astype(float)
#        exdict["dipoles"        ] = [[ self.dipoles        [i][j].astype(float)    for j in range(len(self.dipoles     [i]))] for i in range(len(self.dipoles   ))]
#        exdict["dipolesErr"     ] = [[ self.dipolesErr     [i][j].astype(float)    for j in range(len(self.dipolesErr  [i]))] for i in range(len(self.dipolesErr))]
#        exdict["ampl"           ] = [[[self.ampl           [i][j][k].astype(float) for k in range(len(self.ampl     [i][j]))] for j in range(len(self.ampl   [i]))] for i in range(len(self.ampl   ))]
#        exdict["amplErr"        ] = [[[self.amplErr        [i][j][k].astype(float) for k in range(len(self.amplErr  [i][j]))] for j in range(len(self.amplErr[i]))] for i in range(len(self.amplErr))]
#        exdict["dipole"         ] = [  self.dipole         [i].astype(float)       for i in range(len(self.dipole         ))]
#        exdict["dipoleErr"      ] = [  self.dipoleErr      [i].astype(float)       for i in range(len(self.dipoleErr      ))]
#        exdict["transdensDipole"] = [  self.transdensDipole[i].astype(float)       for i in range(len(self.transdensDipole))]
#        exdict["strength"       ] =    self.strength.astype(float)
#        exdict["strengthErr"    ] =    self.strengthErr.astype(float)
#        exdict["strengthEped"   ] = [  self.strengthEped.astype(float)         for i in range(len(self.strengthEped   ))]
#        exdict["strengthErrEped"] = [  self.strengthEpedErr.astype(float)      for i in range(len(self.strengthEpedErr))]
#        exdict["strengths"      ] = [  self.strengths   [i].astype(float)      for i in range(len(self.strengths      ))]
#        exdict["strengthsErr"   ] = [  self.strengthsErr[i].astype(float)      for i in range(len(self.strengthsErr   ))]
#        exdict["eped"           ] = [  self.eped[i].astype(float)              for i in range(len(self.eped           ))]
#        exdict["epedErr"        ] = [  self.epedErr[i].astype(float)           for i in range(len(self.epedErr        ))]
#        exdict["signifFit"      ] =    self.signifFit.astype(float)
#        exdict["signifErr"      ] =    self.signifErr.astype(float)
#        exdict["signifAng"      ] =    self.signifAng.astype(float)
#        exdict["signifExc"      ] =    self.signifExc.astype(float)
#        exdict["signifRng"      ] =    self.signifRng.astype(float)
#        exdict["signifPha"      ] =    self.signifPha.astype(float)
#        return exdict
